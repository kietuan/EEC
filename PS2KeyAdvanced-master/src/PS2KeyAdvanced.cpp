...
#endif
    _SHIFT, _SHIFT, _CTRL, _CTRL, _ALT, _ALT_GR, _GUI, _GUI};
...
uint16_t translate(void) {
    ...
    if (retdata > 0) {
        if (retdata <= PS2_KEY_CAPS) {  // process lock keys need second make to turn off
            if (PS2_keystatus & _BREAK) {
                PS2_lockstate[retdata] =
                    0;                     // Set received a break so next make toggles LOCK status
        ...
        } else if (retdata >= PS2_KEY_L_SHIFT &&
                   retdata <= PS2_KEY_R_GUI) {  // Update bits for _SHIFT, _CTRL, _ALT, _ALT GR,
                                                // _GUI in status
#if defined(PS2_REQUIRES_PROGMEM)
...
uint8_t PS2KeyAdvanced::getLock() { return (PS2_led_lock); }
